...
class ServerlessPlugin {
    constructor(serverless, options) {
        ...
        this.commands = {
            signer: {
                ...
                options: {
                    function: {
                        usage: 'Specify the function you want to sign ' + '(e.g. "--function main or "-f secondary")',
                        required: true,
                        shortcut: 'f',
                        type: 'string',
                    },
        ...
        this.hooks = {
            'after:aws:package:finalize:saveServiceState': () => this.signLambdas().then(this.signLayers.bind(this)),
            'signer:sign': this.signLambdas.bind(this), // 'before:remove:remove': this.removeResources.bind(this)
        ...
        const globalConfigSchemaProperties = {
            source: {
                type: 'object', properties: {
                    s3: {
                        type: 'object', properties: {
                            bucketName: {type: "string"}, key: {type: "string"}
                        }, required: ["bucketName"]
                    }
                }
            },
        ...
        serverless.configSchemaHandler.defineCustomProperties({
            type: 'object', properties: {
                signer: {
                    type: 'object', properties: globalConfigSchemaProperties, additionalProperties: false
                },
            },
        });
    ...
    generateSignerConfiguration() {
        ...
        // console.log(this.serverless.getCustomResourcesArtifactName())

        const custom_resources = path.join(this.serverless.serviceDir, '.serverless', this.serverless.providers.aws.naming.getCustomResourcesArtifactName())
        const ts = new Date().toISOString();

        try {
            if (fs.existsSync(custom_resources)) {
                const lambda_function = "custom";
        ...
        if (this.serverless.service.package.individually === true) {
            for (let lambda_function in lambda_functions) {
                ...
                signerProcesses[lambda_function] = {
                    signerConfiguration: mergedConfig, packageArtifact: packagePath
                }
                try {
                    // TODO: Remove this check with proper validation
                    if (!signerProcesses[lambda_function].signerConfiguration.source.s3.bucketName) {
    ...
    async signLambdas() {
        ...
        for (let lambda in signerProcesses) {
            this.serverless.cli.log('Signing: ' + lambda);
            ...
            console.log(signItem.signerConfiguration)
            ...
            const {Body} = await this.serverless.providers.aws.request('S3', 'getObject', {
                Bucket: signedCodeLocation.bucketName, Key: signedCodeLocation.key
            })
    ...
    async createS3Bucket(bucketName) {
        ...
        if (this.options.region == "us-east-1") {
            ...
            await this.serverless.providers.aws.request('S3', 'createBucket', {
                Bucket: bucketName, CreateBucketConfiguration: bucketConfiguration
            })
        }

        await this.serverless.providers.aws.request('S3', 'putBucketVersioning', {
            Bucket: bucketName, VersioningConfiguration: {
    ...
    async createSigningProfile(profileName) {
        ...
        const params = {
            platformId: signingPlatforms.platforms[0].platformId, profileName: profileName
        }
    ...
    async addSigningConfigurationToCloudFormation() {
        ...
        const signerProcesses = this.generateSignerConfiguration();
        for (let lambda in signerProcesses) {
            this.serverless.cli.log('Createing signing configuration: ' + lambda);
    ...
    async removeS3Bucket(bucketName) {
        ...
        try {
            ...
            while (s3ObjectsList) {
                ...
                s3Objects.Versions.forEach(item => {
                    s3ObjectsList.push({"Key": item.Key, VersionId: item.VersionId})
                })
                s3Objects.DeleteMarkers.forEach(item => {
                    s3ObjectsList.push({"Key": item.Key, VersionId: item.VersionId})
                })
...
function normalizeResourceName(name) {
    return name.replace(/[^-_]*[-_]*/g, txt => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase()).replace(/-/g, 'Dash').replace(/_/g, 'Underscore');
}
